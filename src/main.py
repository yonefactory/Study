...
# 예외 처리: 최소 두 개의 표현 보장
while len(selected_terms) < 2:
    print("⚠️ 선택된 표현이 부족합니다. 대체 표현을 추가합니다.")
    selected_terms.append(all_terms[len(selected_terms)] if len(selected_terms) < len(all_terms) else "fallback phrase")
